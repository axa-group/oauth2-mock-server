...
describe('helpers', () => {
  ...
  describe('assertIsAlgorithm', () => {
    it.each([
      "nope",
      "SHA1",
    ])('throws on wrong values (%s)', (input) => {
      expect(() => assertIsAlgorithm(input)).toThrow()
    })

    it.each(supportedAlgs)('does not throw on valid algs (%s)', (input) => {
      expect(() => assertIsAlgorithm(input)).not.toThrow()
    })
  })
  ...
  describe('shift', () => {
    ...
    it('does not throw on valid input (%s)', () => {
